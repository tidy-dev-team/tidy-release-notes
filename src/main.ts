...
// Helper to get component set name by ID
function getComponentSetName(componentSetId: string): string {
...
async function buildAggregatedReleaseNotesTable(
  ...
  // Sort sprints by ID (newer first)
  ...
  // Main container with dark background
  const container = figma.createFrame();
  container.name = "Changelog";
  container.layoutMode = "VERTICAL";
  container.primaryAxisSizingMode = "AUTO";
  container.counterAxisSizingMode = "FIXED";
  container.resize(700, 100);
  container.paddingTop = 32;
  container.paddingBottom = 32;
  container.paddingLeft = 0;
  container.paddingRight = 0;
  ...
  changelogSection.counterAxisSizingMode = "AUTO";
  changelogSection.layoutAlign = "STRETCH";
  changelogSection.itemSpacing = 40;
  changelogSection.fills = [];

  // Header row with title + subtitle on left, logo on right
  ...
  // Title: "Changelog" (40px)
  const titleText = figma.createText();
  titleText.fontName = { family: "Inter", style: "Regular" };
  titleText.fontSize = 40;
  titleText.lineHeight = { value: 56, unit: "PIXELS" };
  titleText.characters = "Changelog";
  titleText.fills = [{ type: "SOLID", color: COLORS.textBold }];
...
<path d="M60.0888 0.814839C62.2633 0.814928 64.0263 2.57777 64.0263 4.75228C64.0263 6.92682 62.2634 8.68964 60.0888 8.68973C57.9143 8.6897 56.1514 6.92686 56.1514 4.75228C56.1514 2.57773 57.9143 0.814862 60.0888 0.814839Z" fill="#D4E2F7"/>
  ...
  const logoNode = figma.createNodeFromSvg(logoSvg);
  logoNode.name = "Union";
  logoContainer.appendChild(logoNode);
  headerRow.appendChild(logoContainer);

  changelogSection.appendChild(headerRow);

  // Add each sprint's content
  for (const sprint of sortedSprints) {
    // Sprint section
    const sprintSection = figma.createFrame();
    sprintSection.name = "Sprint";
    sprintSection.layoutMode = "VERTICAL";
    sprintSection.primaryAxisSizingMode = "AUTO";
    sprintSection.counterAxisSizingMode = "AUTO";
    sprintSection.layoutAlign = "STRETCH";
    sprintSection.itemSpacing = 0;
    sprintSection.fills = [];

    // Sprint title row
    const sprintTitleRow = figma.createFrame();
    sprintTitleRow.name = "Title";
    sprintTitleRow.layoutMode = "HORIZONTAL";
    sprintTitleRow.primaryAxisSizingMode = "FIXED";
    sprintTitleRow.counterAxisSizingMode = "AUTO";
    sprintTitleRow.layoutAlign = "STRETCH";
    sprintTitleRow.paddingTop = 8;
    sprintTitleRow.paddingBottom = 8;
    sprintTitleRow.paddingLeft = 24;
    sprintTitleRow.paddingRight = 24;
    sprintTitleRow.fills = [];

    const sprintTitleText = figma.createText();
    sprintTitleText.fontName = { family: "Inter", style: "Medium" };
    sprintTitleText.fontSize = 24;
    sprintTitleText.lineHeight = { value: 32, unit: "PIXELS" };
    sprintTitleText.characters = sprint.name;
    sprintTitleText.fills = [{ type: "SOLID", color: COLORS.textBold }];

    sprintTitleRow.appendChild(sprintTitleText);
    sprintSection.appendChild(sprintTitleRow);

    // Group notes by tag
    const notesByTag = new Map<string, ReleaseNote[]>();
    for (const note of sprint.notes) {
      const existing = notesByTag.get(note.tag) || [];
      existing.push(note);
      notesByTag.set(note.tag, existing);
    }

    // Define tag order (priority)
    const tagOrder = [
      "new_component",
      "enhancement",
      "bug_fix",
    ...
    const sortedTags = Array.from(notesByTag.keys()).sort((a, b) => {
      const indexA = tagOrder.indexOf(a);
      const indexB = tagOrder.indexOf(b);
      return (indexA === -1 ? 999 : indexA) - (indexB === -1 ? 999 : indexB);
    });

    // Create a Log section for each tag
    for (const tag of sortedTags) {
      const tagNotes = notesByTag.get(tag) || [];
      ...
      const badge = createStatusBadge(tag);
      tagRow.appendChild(badge);
      mainContent.appendChild(tagRow);

      // Notes container
      const notesContainer = figma.createFrame();
      notesContainer.name = "Notes";
      notesContainer.layoutMode = "VERTICAL";
      notesContainer.primaryAxisSizingMode = "AUTO";
      notesContainer.counterAxisSizingMode = "AUTO";
      notesContainer.layoutAlign = "STRETCH";
      notesContainer.itemSpacing = 0;
      notesContainer.fills = [];

      // Add each note with component name
      for (const note of tagNotes) {
        const noteRow = figma.createFrame();
        noteRow.name = "who + when";
        noteRow.layoutMode = "HORIZONTAL";
        noteRow.primaryAxisSizingMode = "AUTO";
        ...
        descText.fontSize = 14;
        descText.lineHeight = { value: 24, unit: "PIXELS" };
        descText.characters = note.description;
        descText.fills = [{ type: "SOLID", color: COLORS.textBold }];
        noteRow.appendChild(descText);
      ...
      logRow.appendChild(mainContent);
      sprintSection.appendChild(logRow);
    }

    changelogSection.appendChild(sprintSection);
  }

  container.appendChild(changelogSection);

  // Logo section at bottom (spacer)
  const logoSection = figma.createFrame();
  logoSection.name = "Logo";
  logoSection.layoutMode = "VERTICAL";
...
export default function () {
  ...
  on<PublishSprintReleaseNotesHandler>(
    ...
    async function (sprintId: string) {
      ...
      // Clear existing children and rebuild with ALL sprints
      while (frame.children.length > 0) {
        frame.children[0].remove();
      }

      // Build aggregated table with ALL sprints that have notes
